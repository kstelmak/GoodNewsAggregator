...
namespace NewsAggregatorApp.Services
{
    public class ArticleService:IArticleService
    {
        ...
        private async Task GetRssDataAsync(SourceDto sourceDto, CancellationToken token)
        {
            try
            {
                if (sourceDto.RssUrl != null)
                {
                    using (var xmlReader = XmlReader.Create(sourceDto.RssUrl))
                    {
                        ...
                        //        CategoryDtoId = Guid.NewGuid(),
                        //        CategoryName = it.Categories.First().Name }) //it.Categories is collection but has only 1 element
                        //    .ToArray();
        ...
        private async Task<ArticleDto[]> GetArticlesWithoutTextAsync()
        {
            try
            {
                return (await _mediator.Send(new GetArticlesQuery())).Where(article => string.IsNullOrEmpty(article.Text)).ToArray();
            }
            catch (Exception e)
            {
                throw;
